...
describe("keyDates", () => {
  it("uses the verified 2027–28 FAFSA date for the 2026–27 cycle", () => {
    ...
    expect(fafsa).toMatchObject({ when: "September 23, 2026", start: "2026-09-23", opens: true, title: "The 2027–28 FAFSA opens" });
  });
...
describe("keyDateStatus", () => {
  ...
  it("marks forms as open once they open", () => {
    expect(keyDateStatus(fafsa, "2026-09-22")).toBe("upcoming");
    expect(keyDateStatus(fafsa, "2026-09-23")).toBe("open_now");
  ...
  it("labels the school year", () => {
    ...
    // The 2027–28 FAFSA went live on Sept. 23, 2026.
    expect(year.items.find((i) => i.id === "fafsa")?.status).toBe("open_now");
