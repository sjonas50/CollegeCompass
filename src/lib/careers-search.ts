import { and, ilike, not, sql } from "drizzle-orm";
...
/**
 * What to look for in job titles: alternatives, each a list of words that must all match. Typed
 * words in CAREER_SYNONYMS are swapped for the title words listed there.
 */
function parseCareerQuery(text: string): QueryWord[][] {
  ...
  for (let i = 0; i < typed.length; i++) {
    ...
    segments.push(options ? [...options] : [typed[i]]);
  }
  let alternatives: string[][] = segments.length ? [[]] : [];
  for (const options of segments) {
    alternatives = alternatives.flatMap((alt) => options.map((term) => [...alt, ...term.split(" ")])).slice(0, 16);
  }
  return alternatives.map((words) => words.map((word) => ({ word, forms: wordForms(word) })));
}
 ...
 * ("Dentists, General"). STARTS: the title starts with them ("Nurse Practitioners"). PHRASE: they
 * are next to each other in the title ("Registered Nurses"). OTHER: each word is somewhere in
 * the title, or starts a longer word ("Engineering Managers" for "engineer").
 */
function tierFor(title: string, alt: QueryWord[]): number | null {
  // Words after "Except" name what the career isn't: "Dispatchers, Except Police, Fire, and
  // Ambulance" is no match for "police".
  const words = titleWords(title.replace(/,\s*except\b.*$/i, ""));
  // "Dentists, General" is Dentists; "Art, Drama, and Music Teachers" is a list, not Art.
  const head = /,\s*(?:and|or)\s/i.test(title) ? null : titleWords(title.split(",")[0]);
  const phraseAt = (list: string[], i: number) => alt.every((w, j) => isWord(w, list[i + j]));
  if ((words.length === alt.length && phraseAt(words, 0)) || (head?.length === alt.length && phraseAt(head, 0))) return EXACT;
  if (phraseAt(words, 0)) return STARTS;
  if (words.some((_, i) => phraseAt(words, i))) return PHRASE;
  if (alt.every((w) => words.some((t) => isWord(w, t) || startsWord(w, t)))) return OTHER;
  return null;
}
...
/**
 * Careers whose titles match the words typed, best matches first (see tierFor), then A to Z.
 * Matches whole words, so "art" isn't "Smart" and "actor" isn't "Contractors", with plurals and a
 * few everyday words (CAREER_SYNONYMS). Leaves out O*NET's catch-all "…, All Other" titles.
 */
export async function findCareers(
  ...
  // Titles with any of the words at the start of a word: a short list, ranked here.
  const starts = new Set(alternatives.flat().flatMap((w) => [w.word, ...w.forms]));
  const candidates = await db
    .select({ code: occupations.code, title: occupations.title, jobZone: occupations.jobZone })
    .from(occupations)
    .where(and(sql`${occupations.title} ~* ${`\\m(${[...starts].join("|")})`}`, not(ilike(occupations.title, "%, All Other"))));

  const ranked = candidates
    .map((c) => ({ ...c, tier: Math.min(...alternatives.map((alt) => tierFor(c.title, alt) ?? Infinity)) }))
    .filter((c) => c.tier !== Infinity)
    .sort((a, b) => a.tier - b.tier || a.title.localeCompare(b.title) || a.code.localeCompare(b.code));
