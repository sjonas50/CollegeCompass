...
describe("/account/free-access", () => {
  ...
  it("says when free access can next be renewed", async () => {
    ...
    await grantFreeAccess(db, userId);
    const t = text(await render(FreeAccessPage()));
    expect(t).toMatch(/Your family already has free access until .+\. You can renew it starting .+\./);
  });
});
...
describe("returning from Checkout", () => {
  it("confirms the plan right away", async () => {
    ...
    withStripe(
      ...
      { id: "cs_test_1", mode: "subscription", customer: "cus_1", client_reference_id: householdId },
    );
    const t = text(await render(CheckoutSuccessPage(props<PageProps<"/account/billing/success">>({ session_id: "cs_test_1" }))));
    expect(t).toContain("Thank you! Your plan is active.");
  });

  it("says it's still confirming when Stripe hasn't finished", async () => {
    await signIn("parent");
    const t = text(await render(CheckoutSuccessPage(props<PageProps<"/account/billing/success">>({}))));
    expect(t).toContain("We're confirming your payment.");
  });
